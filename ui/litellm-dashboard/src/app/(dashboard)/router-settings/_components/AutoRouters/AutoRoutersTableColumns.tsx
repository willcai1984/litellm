...
function TypeCell({ row }: { row: AutoRouterRow }) {
  return (
    <Badge variant="secondary" className="font-normal" title={row.strategy}>
      {row.typeLabel}
    </Badge>
  );
}
    ...
    ? [
        {
          ...
          enableSorting: false,
          cell: ({ row }) =>
            row.original.isEditable ? <AutoRouterRowActions row={row.original} onDeleteClick={onDeleteClick} /> : null,
