...
describe("autoRouterRows", () => {
  ...
  it("marks only the two strategies the dashboard has an editor for as editable", () => {
    expect(toAutoRouterRow(complexityDeployment, 0).isEditable).toBe(true);
    expect(toAutoRouterRow(semanticDeployment, 0).isEditable).toBe(true);
  });
  ...
  // Deployments come from two places and only one is writable. A config.yaml row reports
  // db_model false, and the API refuses it: PATCH /model/{id}/update 404s and
  // POST /model/delete 400s. Verified against a live proxy.
  it("marks a config-defined router read-only even when the dashboard understands its shape", () => {
    const row = toAutoRouterRow({ ...complexityDeployment, model_info: { id: "cfg-1", db_model: false } }, 0);

    expect(row.kind).toBe("complexity");
    expect(row.isConfigManaged).toBe(true);
    expect(row.isEditable).toBe(false);
  });
  ...
  it("keeps a DB-created router of a supported strategy editable", () => {
    expect(toAutoRouterRow(complexityDeployment, 0).isEditable).toBe(true);
    expect(toAutoRouterRow(complexityDeployment, 0).isConfigManaged).toBe(false);
  });

  it("needs both axes: a DB-created adaptive router is still not editable", () => {
    const row = toAutoRouterRow(
      {
        model_name: "smart",
        litellm_params: { model: "auto_router/adaptive_router" },
        model_info: { id: "ad-2", db_model: true },
      },
      0,
    );

    expect(row.isConfigManaged).toBe(false);
    expect(row.isEditable).toBe(false);
  });
});
