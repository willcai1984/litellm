...
async def test_ahealth_check_chatgpt_provider_defaults_to_responses_mode():
    ...
    Regression: chatgpt/gpt-5.6-sol (and similar unlisted models) triggered a
    BadRequestError from the ChatGPT backend ('Input must be a list') because the
    health check fell back to 'chat' mode and sent a Chat Completions payload instead
    of the Responses API payload the backend requires.
    """
    import litellm
